...
class GraphiteThresholdCheck extends Check {
	...
	generateUrl(metric, period) {
		return (
			this.ftGraphiteBaseUrl + `format=json&from=-${period}&target=` + metric
		);
	}
	...
	async tick() {
		try {
			...
			const simplifiedResults = results.map((result) => {
				const divideSeriesRegex =
					/divideSeries\(sumSeries\(.*?\),\s?sumSeries\(.*?\)\)/g;
				const asPercentRegex =
					/asPercent\(summarize\(sumSeries\(.*?\),.*?,.*?,.*?\),\s?summarize\(sumSeries\(.*?\),.*?,.*?,.*?\)\)/g;

				if (
					(result.target && asPercentRegex.test(result.target)) ||
					(result.target && divideSeriesRegex.test(result.target))
				) {
					const fetchCountPerTimeUnit = result.datapoints.map((item) =>
						Number(item[0])
					);
				...
				if (result.target && result.target.includes('summarize(sumSeries')) {
					...
					const sumUp = (previousValue, currentValue) =>
			...
			const failingMetrics = simplifiedResults
				.filter((result) => result.isFailing)
			...
			this.checkOutput = failed
				? `In the last ${
						this.samplePeriod
				  }, the following metric(s) have moved ${
						this.direction
				  } the threshold value of ${this.threshold}: ${failingMetrics.join(
						' '
				  )}`
			...
			this.checkOutput =
				'Graphite threshold check failed to fetch data: ' + err.message;
		}
	}
}
