...
function mockGraphite(results) {
	...
	Check = proxyquire('../src/checks/graphiteSpike.check', {
...
describe('Graphite Spike Check', function () {
	...
	it('Should be able to report a successful check of absolute values', function (done) {
		...
		setTimeout(() => {
			expect(mockFetch.firstCall.args[0]).to.contain(
				'from=-10min&format=json&target=summarize(sumSeries(transformNull(next.metric.200)),"10min","sum",true)'
			);
			expect(mockFetch.secondCall.args[0]).to.contain(
				'from=-7d&format=json&target=summarize(sumSeries(transformNull(next.metric.200)),"7d","sum",true)'
			);
	...
	it('Should be able to report a successful check of percentage values', function (done) {
		...
		setTimeout(() => {
			expect(mockFetch.firstCall.args[0]).to.contain(
				'from=-10min&format=json&target=divideSeries(summarize(sumSeries(transformNull(next.metric.200)),"10min","sum",true),summarize(sumSeries(transformNull(metric.*)),"10min","sum",true))'
			);
			expect(mockFetch.secondCall.args[0]).to.contain(
				'from=-7d&format=json&target=divideSeries(summarize(sumSeries(transformNull(next.metric.200)),"7d","sum",true),summarize(sumSeries(transformNull(metric.*)),"7d","sum",true))'
			);
			expect(check.getStatus().ok).to.be.true;
			done();
		});
	...
	it('Should be possible to configure sample and baseline periods', function (done) {
		...
		setTimeout(() => {
			expect(mockFetch.firstCall.args[0]).to.contain(
				'from=-24h&format=json&target=divideSeries(summarize(sumSeries(transformNull(next.metric.200)),"24h","sum",true),summarize(sumSeries(transformNull(metric.*)),"24h","sum",true))'
			);
			expect(mockFetch.secondCall.args[0]).to.contain(
				'from=-2d&format=json&target=divideSeries(summarize(sumSeries(transformNull(next.metric.200)),"2d","sum",true),summarize(sumSeries(transformNull(metric.*)),"2d","sum",true))'
			);
			done();
		});
	});
